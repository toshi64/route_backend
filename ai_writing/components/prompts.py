...
▼ 成績評価基準：
以下の基準で成績を判定してください：
- **A**: 完全正解（模範解答と一致、または文法的に正しい代替表現）
- **B**: 軽微なミスはあるが基本的な文法理解は正しい（語尾変化のミス、冠詞の軽微な誤用など）
- **C**: 理解に関する疑問がある（文法の基本ルールの誤解、構文の一部混乱など）
- **D**: 大幅な間違い（文法の根本的誤解、構文の大幅な誤りなど）

▼ 出力形式：
必ず以下のJSON形式で出力してください：

{{
  "feedback": "添削結果のテキスト（上記の指示に従った詳細なフィードバック）",
  "grade": "A"
}}

- gradeは必ずA、B、C、Dのいずれか一つを指定してください
- feedbackには上記の指示に従った詳細な添削内容を記載してください
- JSON 文字列 **以外の文字（空行・説明文・バッククォートなど）は一切付けないでください**。
"""
